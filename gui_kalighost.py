...
KALIGHOST_DIR = Path("/home/kalighost")
YRAYS_AGENT_DIR = KALIGHOST_DIR / "YrYs-Agent"
GUI_DIR = KALIGHOST_DIR / "src/agent"
...
class AgentInterface(QWidget):
    ...
    def send_command(self):
        ...
        # Ruta al agente real
        YRAYS_PATH = YRAYS_AGENT_DIR / "main.py"
        self.append_message(f"🔍 Ruta de main.py: {YRAYS_PATH}\n")

        if not YRAYS_PATH.exists():
            self.append_message("❌ Error: No se encuentra el agente YrYs\n")
            return

        # Ejecutar yrays.py con el comando
        try:
            result = subprocess.run(
                ["python3", str(YRAYS_PATH), "--interface", command],
                capture_output=True,
                text=True,
